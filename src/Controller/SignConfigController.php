...
final class SignConfigController extends AbstractController
{
    ...
    {
        $signConfigRepository = $this->entityManager->getRepository(SignConfig::class);
    ...
    public function new(Request $request): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $this->entityManager->persist($signConfig);
    ...
    public function edit(Request $request, SignConfig $signConfig): Response
    {
        $form = $this->createForm(SignConfigType::class, $signConfig);
        $form->handleRequest($request);
    ...
    public function delete(Request $request, SignConfig $signConfig): Response
    {
        if ($this->isCsrfTokenValid('delete'.$signConfig->getId(), $request->getPayload()->getString('_token'))) {
            $this->entityManager->remove($signConfig);
            $this->entityManager->flush();
        }
